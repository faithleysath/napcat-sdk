...
async def process_file(
    client: AsyncOpenAI, sem: asyncio.Semaphore, ts_path: str
...
def run_ruff_formatting(target_dir: str):
    ...
    if not ruff_exe:
        logger.warning(
            "⚠️ Ruff not found in PATH. Skipping formatting. (pip install ruff)"
        )
        return
...
async def main():
    ...
    ts_files = [
        f
        for f in glob.glob(os.path.join(TS_SOURCE_DIR, "*.ts"))
